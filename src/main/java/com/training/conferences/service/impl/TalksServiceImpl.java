...
import com.training.conferences.adaptors.persistence.TalkDao;
...
import com.training.conferences.exception.EnoughTalkOnConferenceException;
import com.training.conferences.exception.InvalidConferenceIdException;
import com.training.conferences.exception.TalkAlreadyExistInConferenceException;
import com.training.conferences.exception.TalkLess30DaysException;
...
public class TalksServiceImpl implements TalksService {
  ...
  public long addTalkToConference(Long conferencesId, CreateNewTalkDto createNewTalkDto) {
    ...
    var duplicateTalk =
        conferenceTalk.stream()
            .map(TalkEntity::getName)
            .filter(s -> s.equals(createNewTalkDto.getName()))
            .findFirst();

    if (duplicateTalk.isPresent())
      throw new TalkAlreadyExistInConferenceException(
          MessageFormat.format(
              "Talk {0} already exist in conference {1}",
              createNewTalkDto.getName(), conferenceEntity.getName()));

    int size =
        conferenceTalk.stream()
            .filter(
                talkEntity -> talkEntity.getSpeakerName().equals(createNewTalkDto.getSpeakerName()))
            .toList()
            .size();

    if (size > 3)
      throw new EnoughTalkOnConferenceException(
          MessageFormat.format(
              "Enough talk, no more 3 speech by speaker. Speaker name={0}",
              createNewTalkDto.getSpeakerName()));

    LocalDate startDate = conferenceEntity.getStartDate();

    if (ChronoUnit.DAYS.between(LocalDate.now(), startDate) <= 30)
      throw new TalkLess30DaysException(
          MessageFormat.format("Conference {0} start less 30 days", conferenceEntity.getName()));

    var taskEntity =
        new TalkEntity(
            createNewTalkDto.getName(),
            createNewTalkDto.getDescription(),
            createNewTalkDto.getSpeakerName(),
            createNewTalkDto.getReportType(),
            conferenceEntity);
  ...
  private List<TalkDto> toDto(Stream<TalkEntity> books) {
    return books.map(TalksServiceImpl::toDto).collect(toList());
  }
  ...
  public static TalkDto toDto(TalkEntity talkEntity) {
    return new TalkDto(
        talkEntity.getId(),
        talkEntity.getName(),
        talkEntity.getSpeakerName(),
        talkEntity.getDescription(),
        talkEntity.getReportType());
  }
}
